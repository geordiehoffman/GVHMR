...
class Tracker:
    ...
    def track(self, image_path):
        """Run detection (not real tracking) on a single image."""
        assert is_image_file(image_path), f"Only image files are supported. Got: {image_path}"
        img = Image.open(image_path).convert("RGB")
        results = self.yolo.predict(img, device="cuda", conf=0.5, classes=0, verbose=False)
        boxes = results[0].boxes
        track_history = [[
            {"id": i, "bbx_xyxy": boxes.xyxy[i].cpu().numpy()}
            for i in range(len(boxes.xyxy))
        ]]
        return track_history
    
    def get_one_track(self, image_path):
        """Return bounding box (xyxy) of first detection in the image."""
        track_history = self.track(image_path)

        if not track_history or not track_history[0]:
            raise ValueError(f"No detections found in image: {image_path}")

        bbx_xyxy = torch.tensor([det["bbx_xyxy"] for det in track_history[0]])  # (N, 4)
        return bbx_xyxy[0]  # Return first detection only
